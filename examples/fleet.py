...
scope = MemoryScope(
    agent_id="rail-example-writer", fleet_id="rail-examples", visibility=Visibility.TEAM
)
...
with RestMemoryStore.from_env() as store:
    ...
    if turn.degraded or not turn.writes or turn.writes[0].status != "written":
        raise RuntimeError(
            "Write did not complete; inspect the turn errors and backend configuration"
        )
